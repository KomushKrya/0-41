...
**Таймеры игрока включены.** В событиях `IncidentCreated`, `MapMarkerSpawned` и
`RadioTriggered` приходит обычная длительность из `data/config.json`; интерфейс рисует
обратный отсчёт для звонка, метки и ответа по рации. Значение `0` по-прежнему означает
«без ограничения» для смен, где это явно задано конфигурацией.
