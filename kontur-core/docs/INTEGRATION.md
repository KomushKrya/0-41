...
После интеграции контент игры живёт в `data/`. Копия для автономного прогона
(`kontur-core/content/`) удалена — она разошлась с `data/` (старые имена
характеристик, несовместимый формат `radio.json`) и харнесс без явного
`--content` натыкался на неё первой. Харнесс всё ещё ищет сначала `data/`,
потом `content/`, но запускать его теперь нужно строго через `--content data`.
