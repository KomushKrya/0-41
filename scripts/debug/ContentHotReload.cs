...
public partial class ContentHotReload : Node
{
	...
	private bool RunConverter()
	{
		...
		// Предупреждение, а не ошибка. Конвертер — инструмент авторов текста;
		// у того, кто просто запускает игру, питона в PATH может не быть, и
		// красный стек в консоли на старте выглядит как поломка проекта.
		// Код 9009 в Windows означает ровно одно: команда не найдена.
		if (code == 9009)
		{
			GD.PushWarning(
				"[текст] конвертер пропущен: питон не найден в PATH. "
				+ "Игра пойдёт на уже собранных текстах из content/localisation. "
				+ "Пересборка текстов нужна только тем, кто правит content/raw.");
			return false;
		}
